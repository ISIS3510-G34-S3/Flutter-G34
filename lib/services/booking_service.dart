...
class BookingService {
  ...
  Future<bool> createBookingOfflineCapable(Booking booking) async {
    ...
    if (bookingData['startsAt'] is Timestamp) {
      bookingData['startsAt'] =
          (bookingData['startsAt'] as Timestamp).toDate().toIso8601String();
    }
    if (bookingData['endsAt'] is Timestamp) {
      bookingData['endsAt'] =
          (bookingData['endsAt'] as Timestamp).toDate().toIso8601String();
    }
    if (bookingData['createdAt'] is Timestamp) {
      bookingData['createdAt'] =
          (bookingData['createdAt'] as Timestamp).toDate().toIso8601String();
    }
  ...
  Stream<int> getBookingsCountStream(String experienceId) async* {
    final prefs = await SharedPreferences.getInstance();
    final cacheKey = 'booking_count_$experienceId';
    ...
    _connectivitySubscription = connectivity.onConnectivityChanged.listen(
      (List<ConnectivityResult> results) async {
        ...
        if (isConnected) {
          if (!_isSyncing) {
            print(
                '🌐 Connectivity restored (BookingService), checking pending bookings...');
            syncPendingBookings();
          }
        }
      },
  ...
  Future<void> _checkAndSyncOnStartup() async {
    ...
    if (isOnline && !_isSyncing) {
      print(
          '📱 App started online (BookingService), checking pending bookings...');
  ...
  Future<void> syncPendingBookings() async {
    ...
    try {
      ...
      final pendingBookings =
          pending.where((op) => op.type == 'create_booking').toList();
      ...
      for (final operation in pendingBookings) {
        try {
          ...
          if (bookingData['startsAt'] is String) {
            bookingData['startsAt'] =
                Timestamp.fromDate(DateTime.parse(bookingData['startsAt']));
          }
          if (bookingData['endsAt'] is String) {
            bookingData['endsAt'] =
                Timestamp.fromDate(DateTime.parse(bookingData['endsAt']));
          }

          bookingData['createdAt'] = FieldValue.serverTimestamp();
          if (bookingData['travelerID'] is String) {
            bookingData['travelerID'] =
                _firestore.doc(bookingData['travelerID']);
          }
